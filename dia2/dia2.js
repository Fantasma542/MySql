...
async function main(){
    try{
        ...
        // let sql = "SELECT s.title AS Matematicas, COUNT(t.teacher_id) AS NumeroDeProfesores FROM subject s LEFT JOIN subject_teacher t ON s.subject_id = t.subject_id GROUP BY s.title"

        // let sql = "SELECT student_id, mark FROM marks WHERE (student_id >= 1 AND student_id <= 20) OR (mark > 8 AND date BETWEEN \"2022-01-01\" AND \"2022-09-31\")"
